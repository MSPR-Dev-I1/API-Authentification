...
from app.compute.compute import get_deactivated_tokens, get_role
from app.tokken.tokken import verify_validity, verify_access, encode_jwt
from app.database.premier_schema import Role
...
async def deploy_token(request:TokenRequest,db: Session = Depends(get_db)):
    """
        This API recieve a role id.
        It returns a new token correspondinf to that role.
    """
    try:
        role: Role = get_role(db, request.role)
        if role is None:
            raise HTTPException(status_code=404,
                                detail="Cannot deliver any token for a role that does not exist.")
